...
export function ManageCasesClient({
  ...
  const [detailsOpen, setDetailsOpen] = useState(cases.length === 0);

  useEffect(() => {
    if (cases.length === 0) setDetailsOpen(true);
  }, [cases.length]);
  ...
  return (
    ...
    >
      <summary className="flex cursor-pointer select-none items-center justify-between px-4 py-3 text-sm font-medium">
        ...
        <span className="text-xs text-white/40">{detailsOpen ? "hide" : "show"}</span>
      </summary>
        ...
        <ul className="divide-y divide-white/5">
          {sorted.map((c) => {
            ...
            return (
              ...
              >
                <div
                  className="hidden h-10 w-10 shrink-0 overflow-hidden rounded sm:flex items-center justify-center"
                  style={{ backgroundColor: RARITY_BG[mr] ?? RARITY_BG["Mil-Spec"] }}
                >
                  {c.imageUrl ? (
                    <img
                      src={c.imageUrl}
                    ...
                    <span className="text-xs font-bold" style={{ color: RARITY_COLORS[mr] }}>
                      {c.name[0]}
                    </span>
                  )}
                </div>
          ...
          {!cases.length && (
            <li className="px-4 py-3 text-sm text-white/40">
              Empty cache. Use the &ldquo;Manual JSON paste&rdquo; panel to load cases.
            </li>
          )}
        </ul>
        {cases.length > 0 && (
