  ...
  const totalSelected = useMemo(
    () =>
      Object.entries(selected)
        ...
        .reduce((acc, [slug, _on]) => acc + (counts[slug] ?? 0), 0),
    [selected, counts],
  );
    ...
    <div className="space-y-5">
      ...
      <header className="flex items-baseline justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            className="w-56 rounded border border-white/10 bg-[#0b0e14] px-3 py-1 text-sm"
          />
          <button
            onClick={sendToSim}
      ...
      {filtered.length === 0 ? (
        <div className="rounded-lg border border-white/10 bg-white/5 p-6 text-sm text-white/60">
          No cases match. Use the manual paste fallback above to load more cases.
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3">
          {filtered.map((c) => {
            ...
            return (
              <div
                key={c.slug}
                className="relative flex flex-col rounded-xl border border-white/10 p-4 transition"
                style={{
              ...
              >
                <div className="flex items-start gap-2">
                  ...
                  />
                  <Link href={`/cases/${c.slug}`} className="flex-1 hover:text-amber-400">
                    ...
                    <div className="mt-1 text-xs text-white/50">
                      EV <span className="text-white/80">{ev(c).toFixed(2)}</span> · edge{" "}
                      <span className={edgeColor}>{edge >= 0 ? "+" : ""}{edge.toFixed(2)}%</span>
                    </div>
                    <div className="mt-1 text-xs text-white/50">
